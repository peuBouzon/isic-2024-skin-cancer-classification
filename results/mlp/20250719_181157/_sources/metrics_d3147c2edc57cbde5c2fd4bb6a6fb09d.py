...
def plot_precision_recall_curve(probs, y_true, save_path, min_recall=0.8):
    ...
    if len(high_recall_idxs) > 0:
        ...
        plt.axvline(x=min_recall, color='r', linestyle='--', 
                   label=f'Limiar de Sensibilidade = {min_recall}')
    ...
    plt.plot(target_recall_actual, target_precision, 'ro', markersize=8, 
            label=f'Limiar = {target_threshold:.4f} (P={target_precision:.4f}, S={target_recall_actual:.4f})')
...
def plot_precision_recall_curves(methods_names, 
    ...
    for i, (method_name, probs, y_true, (color, color_fill_between)) in enumerate(zip(methods_names, methods_probs, methods_labels, colors)):
        ...
        plt.step(recall, precision, color=color, where='post', lw=2, 
                label=f'Curva PR {method_name} (AP = {average_precision:.4f})')
        ...
        plt.plot(target_recall_actual, target_precision, f'{color}o', markersize=8, 
                label=f'P={target_precision:.4f}, TPR={target_recall_actual:.4f} (Limiar = {target_threshold:.4f})')
...
def plot_rocs_with_partial_aucs(methods_names, 
    ...
    for i, (method_name, probs, labels, (color, color_fill_between)) in enumerate(zip(methods_names, methods_probs, methods_labels, colors)):
        ...
        #plt.figure(figsize=(10, 8))
        plt.plot(fpr, tpr, color=color, lw=2, label=f'ROC {method_name} (pAUC @ 80% TPR = {get_partial_auc(probs, labels):.4f})')
        ...
        plt.plot(target_fpr, target_tpr, f'{color}o', markersize=8, 
                label=f'TPR={target_tpr:.4f}, FPR={target_fpr:.4f} (Limiar = {target_threshold:.4f})')
